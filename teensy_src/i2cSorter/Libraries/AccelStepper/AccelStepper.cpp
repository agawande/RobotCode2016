...
void AccelStepper::setOutputPins(uint8_t mask)
{
    ...
    uint8_t i;
    for (i = 0; i < numpins; i++)
	digitalWriteFast(_pin[i], (mask & (1 << i)) ? (HIGH ^ _pinInverted[i]) : (LOW ^ _pinInverted[i]));
}
...
{   
    ...
    if (_enablePin != 0xff)
        digitalWriteFast(_enablePin, LOW ^ _enableInverted);
}
...
void    AccelStepper::enableOutputs()
{
    ...
    if (_enablePin != 0xff)
    {
        ...
        digitalWriteFast(_enablePin, HIGH ^ _enableInverted);
    }
}
